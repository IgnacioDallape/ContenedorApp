...
```
contenedorapp/
├── index.html              # Solo shell HTML (login, appshell, templates IP y CL)
...
│   ├── container-loader.js # Estado CL, UI del form, renderLoader, zonas de prioridad
...
`auth.js` → `packing.js` → `container-loader.js` → `importapro-calc.js` → `importapro-products.js` → `importapro-ncm.js` → `importapro-sim.js` → `three-scene.js` → `three-render.js` → `catalog.js` → `app.js`
