  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F4EE', fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ background: '#8D7966', color: '#fff', padding: '16px 32px' }}>
        <div style={{ fontSize: 11, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", letterSpacing: 2, opacity: 0.8, marginBottom: 4 }}>IMPORTAPRO — PALLET COMPARTIDO</div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 10, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", color: st.color, letterSpacing: 1, marginBottom: 2 }}>ESTADO</div>
          ...
          {pallet.tracking_url && (
            <a href={pallet.tracking_url} target="_blank" rel="noopener noreferrer"
              style={{ padding: '8px 16px', background: '#fff', color: '#8D7966', border: '1.5px solid #8D7966', borderRadius: 8, textDecoration: 'none', fontFamily: "'Inter', system-ui, -apple-system, sans-serif", fontSize: 11, letterSpacing: 1, whiteSpace: 'nowrap' }}>
              🔗 Tracking →
            </a>
          )}
        </div>
        ...
        {active && (
          <div style={{ marginBottom: 20, background: '#fff', border: '1px solid #E8E0D8', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ background: '#F0EBE3', padding: '10px 16px', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif", fontSize: 10, color: '#8D7966', letterSpacing: 1 }}>VISTA 3D — SOLO LECTURA</span>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {results.map((r, i) => (
                  <button key={i} onClick={() => setActiveIdx(i)}
                    style={{ padding: '4px 10px', fontSize: 11, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", borderRadius: 5, cursor: 'pointer', border: `1.5px solid ${activeIdx === i ? '#8D7966' : '#E8E0D8'}`, background: activeIdx === i ? '#8D7966' : 'transparent', color: activeIdx === i ? '#fff' : '#8D7966' }}>
          ...
          return (
            <div key={i} style={{ background: '#fff', borderRadius: 10, border: '1px solid #E8E0D8', marginBottom: 16, overflow: 'hidden' }}>
              <div style={{ background: '#F0EBE3', padding: '12px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ fontSize: 11, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", color: '#8D7966' }}>
        ...
        {/* Footer */}
        <div style={{ textAlign: 'center', padding: '24px 0', color: '#8D7966', fontSize: 11, fontFamily: "'Inter', system-ui, -apple-system, sans-serif", letterSpacing: 1 }}>
